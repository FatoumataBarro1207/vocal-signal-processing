...
git clone https://github.com/TON\_USERNAME/vocal-signal-processing.git
...
│   └── fft\_filter.py          # FFT · masque H(f) · IFFT · graphiques
...
Passe-bande : H(f) = 1   si f\_min ≤ |f| ≤ f\_max  ,  0 sinon

Coupe-bande : H̄(f) = 1 − H(f)
...
freqs = scipy.fft.fftfreq(N, d=1.0/sample\_rate)
...
signal\_filtre = np.real(scipy.fft.ifft(X\_filtre))
...
| `soundfile` | ≥ 0.12 | Lecture / écriture WAV (PCM\_16, PCM\_32) |

| `matplotlib` | ≥ 3.8 | Génération graphiques (mode serveur Agg) |

| `pydub` | ≥ 0.25 | Conversion formats audio (MP3, OGG → WAV) |
...
\- Affichage signal temporel x(t) et spectre |X(f)|

\- Filtre rectangulaire configurable (fmin, fmax, type)
...
\*\*\[Fatoumata BARRO]\*\* \& \*\*\[Mouhamed Drame]\*\*

DIC2 — Département de Génie Informatique — ESP/UCAD
...
\- \[Web Audio API — MDN](https://developer.mozilla.org/fr/docs/Web/API/Web\_Audio\_API)
